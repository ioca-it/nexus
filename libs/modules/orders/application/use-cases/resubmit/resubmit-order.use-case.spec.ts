...
const UPDATED_AT = new Date('2026-08-01T15:30:00.000Z');
...
function changesRequestedOrder(): Order {
  return Order.create({
    id: ORDER_ID,
...
function orderWithStatus(status: OrderStatus): Order {
  const order = changesRequestedOrder();
  return status === ORDER_STATUS.CHANGES_REQUESTED
    ? order
    : order.transitionTo(status, new Date('2026-07-30T13:00:00.000Z'));
}

function repository(
  order: Order | null = changesRequestedOrder(),
...
describe('ResubmitOrderUseCase', () => {
  ...
  it('is exported publicly and its request contract requires actor and id', () => {
    const request: ResubmitOrderRequest = { actor: actor(), id: ORDER_ID };

    expect(OrdersPublicApi.ResubmitOrderUseCase).toBe(ResubmitOrderUseCase);
  ...
  it('normalizes id, finds once, and uses the documented not-found error', async () => {
    ...
    const useCase = new ResubmitOrderUseCase({
      repository: repo,
  ...
  it('uses the real CHANGES_REQUESTED state, RESUBMIT action and shared workflow', async () => {
  ...
  it('transitions immutably, preserves the snapshot and persists updatedOrder once', async () => {
    const original = changesRequestedOrder();
    ...
    expect(original.status).toBe(ORDER_STATUS.CHANGES_REQUESTED);
    expect(original.updatedAt).not.toEqual(UPDATED_AT);
  ...
  it('ignores arbitrary roles when permission and ownership are valid', async () => {
    const repo = repository();
    const useCase = new ResubmitOrderUseCase({
      repository: repo,
      stateTransition: stateTransition(),
  ...
  ])('denies resubmit from real %s without effects', async (status) => {
  ...
  it.each([
    ...
    async (_name, resultFromPipeline) => {
      ...
      await expect(
        useCase.execute({ actor: actor(), id: ORDER_ID }),
      ).rejects.toThrow('Order resubmit pipeline result is inconsistent');
      expect(clock).not.toHaveBeenCalled();
      expect(transitionTo).not.toHaveBeenCalled();
      expect(repo.update).not.toHaveBeenCalled();
    },
  ...
  it('contains no external integrations or commercial recalculation', () => {
    const source = readFileSync(
      join(__dirname, 'resubmit-order.use-case.ts'),
      'utf8',
    );
    const requestContract = source.slice(
      source.indexOf('export interface ResubmitOrderRequest'),
      source.indexOf('export interface ResubmitOrderResult'),
    );
    ...
    expect(source).not.toMatch(
      /currentState\s*:\s*ORDER_STATUS\.CHANGES_REQUESTED/,
    );
  });
});
